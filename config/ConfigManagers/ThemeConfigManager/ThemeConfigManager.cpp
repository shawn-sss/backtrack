...
bool ThemeConfigManager::isDarkTheme() {
#ifdef Q_OS_WIN
...
AppTheme ThemeConfigManager::currentTheme() {
    return _currentTheme;
}

// Gets stored user theme preference
UserThemePreference ThemeConfigManager::getUserThemePreference() {
    return ConfigDirector::getInstance().getThemePreference();
}

// Sets and applies new user theme preference
void ThemeConfigManager::setUserThemePreference(UserThemePreference preference) {
    ConfigDirector::getInstance().setThemePreference(preference);
    applyTheme();
}

// Applies theme based on user or system setting
void ThemeConfigManager::applyTheme() {
    auto& config = ConfigDirector::getInstance();
    const UserThemePreference preference = config.getThemePreference();
...
void ThemeConfigManager::installEventFilter(QObject*) {
#ifdef Q_OS_WIN
    if (!eventFilter) {
