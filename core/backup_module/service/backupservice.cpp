...
void BackupService::initializeBackupRootIfNeeded() {
    const QString configFilePath = QDir(backupRootPath).filePath(QStringLiteral("%1/%2").arg(AppConfig::k_BACKUP_SETUP_FOLDER, AppConfig::k_BACKUP_SETUP_INFO_FILE));
    ...
    backupConfig["app_name"] = AppInfo::k_APP_DISPLAY_TITLE;
    backupConfig["app_author"] = AppInfo::k_AUTHOR_NAME;
...
BackupStatus BackupService::scanForBackupStatus() const {
    const QDir configDir(QDir(backupRootPath).filePath(AppConfig::k_BACKUP_SETUP_FOLDER));
    if (!configDir.exists()) return BackupStatus::None;
    bool validLogs = QDir(configDir.filePath(AppConfig::k_BACKUP_LOGS_FOLDER)).exists();
    bool validConfig = QFile::exists(configDir.filePath(AppConfig::k_BACKUP_SETUP_INFO_FILE));
...
QJsonObject BackupService::getLastBackupMetadata() const {
    const QDir logsDir(QDir(backupRootPath).filePath(QStringLiteral("%1/%2").arg(AppConfig::k_BACKUP_SETUP_FOLDER, AppConfig::k_BACKUP_LOGS_FOLDER)));
    QFileInfoList logFiles = logsDir.entryInfoList({"*_" + AppConfig::k_BACKUP_LOGS_FILE}, QDir::Files, QDir::Time);
...
void BackupService::createBackupSummary(const QString &backupFolderPath, const QStringList &selectedItems, qint64 backupDuration) {
    const QString logsFolderPath = QDir(backupRootPath).filePath(QStringLiteral("%1/%2").arg(AppConfig::k_BACKUP_SETUP_FOLDER, AppConfig::k_BACKUP_LOGS_FOLDER));
    if (!QDir(logsFolderPath).exists()) {
        QDir().mkpath(logsFolderPath);
    }
    QString logFileName = QFileInfo(backupFolderPath).fileName() + "_" + AppConfig::k_BACKUP_LOGS_FILE;
...
int BackupService::getBackupCount() const {
    const QString logsFolderPath = QDir(backupRootPath).filePath(QStringLiteral("%1/%2").arg(AppConfig::k_BACKUP_SETUP_FOLDER, AppConfig::k_BACKUP_LOGS_FOLDER));
    return QDir(logsFolderPath).entryList({"*_" + AppConfig::k_BACKUP_LOGS_FILE}, QDir::Files).size();
}
...
quint64 BackupService::getTotalBackupSize() const {
    ...
    const QDir logsDir(QDir(backupRootPath).filePath(QStringLiteral("%1/%2")
                                                         .arg(AppConfig::k_BACKUP_SETUP_FOLDER, AppConfig::k_BACKUP_LOGS_FOLDER)));
    const QFileInfoList logFiles = logsDir.entryInfoList({"*_" + AppConfig::k_BACKUP_LOGS_FILE}, QDir::Files);
